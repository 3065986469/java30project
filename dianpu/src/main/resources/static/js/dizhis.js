...
// 获取省
var provlevels = document.getElementById('provlevels');
// 获取市
var citylevels = document.getElementById('citylevels');
// 获取区
var arealevels = document.getElementById('arealevels');
// 循环第一步,把省循环进select
for (var i = 0; i < city.length; i++) {
    provlevels.options[provlevels.length] = new Option(city[i].name);
    // 循环第二步,把所有的市都循环进select
    provlevels.onchange = function(){
        citylevels.options.length = 0;
        citylevels.options[citylevels.length] = new Option("请选择城市");
        for (var j = 0; j < city[provlevels.selectedIndex-1].city.length; j++) {
            citylevels.options[citylevels.length] = new Option(city[provlevels.selectedIndex-1].city[j].name)
        }
    };
    citylevels.onchange = function(){
        arealevels.options.length = 0;
        arealevels.options[arealevels.length] = new Option("请选择区/县");
        for (var k = 0; k < city[provlevels.selectedIndex-1].city[citylevels.selectedIndex-1].area.length; k++) {
            arealevels.options[arealevels.length] = new Option(city[provlevels.selectedIndex-1].city[citylevels.selectedIndex-1].area[k]);
        }
    };
    arealevels.onchange = function () {
        // 获取固定地址的输入框
        var citysinfos = document.getElementById('citysinfos');
        // 省
        var index = provlevels.selectedIndex ;
        var provinfo = provlevels.options[index].value;
        // 市
        var index1 = citylevels.selectedIndex ;
        var cityinfo = citylevels.options[index1].value;
        // 区
        var index2 = arealevels.selectedIndex ;
        var areainfo = arealevels.options[index2].value;
        // 将省市区写到固定的输入框不能修改
        citysinfos.value = (provinfo+"-"+cityinfo+"-"+areainfo);
    }
}